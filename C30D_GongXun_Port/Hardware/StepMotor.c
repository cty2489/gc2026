 ...
 *返回类型:无
 *备注:采用TIM6定时中断+DMA发送
 *备注:C30D移植版采用PD5(USART2-Tx),默认定时1.5ms
 */
void StepMotor_HardwareInit(void)
{
	...
	DMA_InitStructure.DMA_PeripheralBaseAddr=(uint32_t)&(USART2->DR);//外设地址(USART的DR数据寄存器)
...
void StepMotor_DMAReset(uint8_t Size)
{
	...
	DMA_SetCurrDataCounter(DMA1_Stream6,Size);//恢复传输计数器的值
	DMA_Cmd(DMA1_Stream6,ENABLE);//使能DMA1的数据流6
}
