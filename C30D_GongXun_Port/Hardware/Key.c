...
volatile FlagStatus Key_Status=RESET;//按键状态 按下进行RESET/SET切换

/*
 *函数简介:按键定时器扫描初始化
 *参数说明:无
 *返回类型:无
 *备注:C30D原生KEY引脚PE0,默认定时20ms
 */
void Key_TIMScanInit(void)
{
	...
	GPIO_Init(GPIOE,&GPIO_InitStructure);//初始化GPIO_PE0
 ...
 *参数说明:无
 *返回类型:无
 *备注:在TIM9定时中断调用,20ms扫描一次,C30D原生KEY按下为低电平
 */
void Key_GetStatus_Callback(void)
{
	...
	Key_NowStatus=GPIO_ReadInputDataBit(GPIOE,GPIO_Pin_0);
	
	if(Key_LastStatus==1 && Key_NowStatus==0)Key_Status=!Key_Status;//判断为下降沿,即按键按下
}
